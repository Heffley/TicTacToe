...
import com.example.gosen.tictac.Manager;
...
public class TicTacBoardManager implements Serializable, Manager {
    ...
    public TicTacBoardManager DeepCopy() {
        TicTacBoardManager boardManager = new TicTacBoardManager();
        boardManager.p1Wins = this.p1Wins;
        boardManager.strategy = this.strategy;
        boardManager.moveCounter = this.moveCounter;
        boardManager.board = new TicTacBoard(this.getBoard());

        boardManager.game_position = this.game_position;
        return boardManager;
    }
    ...
    /**
     * get the last move position ob game
     */
    ...
    /**
     * Make a move for recursive call at position in the board, swapping tiles as appropriate.
     *
     * @param position the position
     * @return the current player turn
     */
    ...
    boolean isValidTap(int position) {
        if (moveCounter == board.getRows() * board.getCols()) {
             return false;
        }
